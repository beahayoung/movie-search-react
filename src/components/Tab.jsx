...
const Tab = ({sendData}) => {
    ...
    return (
        <>
            ...
            <GenreWrap>
                <SelectWrap>
                    <OptionPick className='option_pick' onClick={() => setSelectOpen(!selectOpen)}>
                        {options[selectValue]}
                        ...
                        <OptionWrap>
                            {
                                Object.entries(options).map(([value, label]) => (
                                    <Option key={value} className={`option ${selectValue === value ? "pick" : ""}`} onClick={() => handleOptionValue(value)}>{label}</Option>
                                ))
                            }
                        </OptionWrap>
                    }
                ...
                <GenreListWrap>
                    {sortedMovied.map((genre) => (
                        <GenreImgWrap className={`genre_img_wrap ${genre.adult ? "adult" : ""}`} key={genre.id}>
                            <GenreListImg src={`https://image.tmdb.org/t/p/original/${genre.poster_path}`} />
                        </GenreImgWrap>
                    ))}
                </GenreListWrap>
                  {
